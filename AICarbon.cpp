...
#define MATCH_SPARE 8      //how much is time spared for the rest of game
...
void AICarbon::yourTurn(int &x, int &y, int depth, int time) {
	...
	if (depth > 0) {
		...
		for (depth = 2; depth <= 99; depth += 2) {
			...
			t1 = getTime();
			td = t1 - t0;
			if (terminateAI || t1 + TIMEOUT_PREVENT * td - stopTime() >= 0) break;

			if (nSearched != prevSearched)
				WriteLog(best.value, nSearched, td == 0 ? 0 : nSearched / td, depth / 2, x, y, true);
			prevSearched = nSearched;

			if (best.value <= -WIN_MIN || best.value >= WIN_MIN) {
				break;
			}
		}
	}
	...
	WriteLog(best.value, nSearched, td == 0 ? 0 : turnSearched / td, depth / 2, x, y, false);
...
OXMove AICarbon::minimax(int h, bool root, int alpha, int beta) {
	...
	if (nSt[opp][A] > 0) h--;
	else if (nSt[opp][B] > 0) h -= 2;
	else if (nSt[who][C] > 0 || nSt[who][F] > 0) h -= 3;
	else {
		if (h <= 12 && h % 2 == 0 && alpha > -WIN_MIN && beta < WIN_MIN) {
			const int margin = 70;
			int eval = evaluate();
			if (eval - h * margin / 4 >= beta) return OXMove(0, 0, eval);
		}
		h -= 4;
	}
